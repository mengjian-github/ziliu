 ...
 * 仅处理视频发布逻辑
 */
class XiaohongshuVideoPlugin extends BasePlatformPlugin {
  constructor(config) {
    super(config);
    this.platformType = 'video';
    console.log('📖 小红书视频插件初始化完成');
  }
  ...
  async fillContent(data) {
    ...
    try {
      ...
      // 视频内容优先使用 videoTitle / videoDescription
      ...
      // 填充封面（视频）
      const coverUrl = data.coverImage || data.coverImage169 || data.coverImage43;
      if (coverUrl) {
        results.cover = await this.fillCover(elements, coverUrl);
      ...
      } else if (data.coverSuggestion) {
        console.warn('⚠️ 未提供封面图片，仅有封面建议，无法自动上传');
      }
    ...
    try {
      ...
      for (const tagElement of recommendTags) {
        ...
        if (tagText === normalizedTarget ||
          ...
          if (!this.isElementVisible(tagElement)) continue;
...
if (typeof window !== 'undefined' && window.ZiliuPlatformRegistry) {
  const configs = (window.ZiliuPluginConfig?.platforms || [])
    .filter(p => p.id === 'xiaohongshu' && p.enabled);

  configs.forEach((config) => {
    // 避免重复注册
    if (window.ZiliuPlatformRegistry.get(config.id)) return;

    const plugin = new XiaohongshuVideoPlugin(config);
    window.ZiliuPlatformRegistry.register(plugin);
    console.log(`📖 小红书视频插件已注册到平台注册中心: ${config.id}`);
  });
}
